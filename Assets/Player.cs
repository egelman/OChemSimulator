...
public class Player : SyncState
{
	...
	void Start()
	{
		moleculeManager = GameObject.FindObjectOfType<MoleculeManager>();
	...
	private void Update()
	{
		if (InputMan.Button1Down(Side.Left) || Input.GetKeyDown(KeyCode.Space))
		{
			VelNetManager.NetworkInstantiate(moleculeManager.addCarbonAtomPrefab.name, r.leftHand.transform.position + r.leftHand.transform.forward * .5f, r.leftHand.transform.rotation);
		}
		if (InputMan.Button1Down(Side.Right))
		{
			VelNetManager.NetworkInstantiate(moleculeManager.addNitrogenAtomPrefab.name, r.rightHand.transform.position + r.rightHand.transform.forward * .5f, r.rightHand.transform.rotation);
		}
		if (InputMan.Button2Down(Side.Left))
		{
			VelNetManager.NetworkInstantiate(moleculeManager.addOxygenAtomPrefab.name, r.leftHand.transform.position + r.leftHand.transform.forward * .5f, r.leftHand.transform.rotation);
		}
	}
